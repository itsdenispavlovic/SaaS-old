...
<input type="hidden" name="parent_id" id="parent_id" value="{{ (!empty($node->parent_id) ? $node->parent_id : (request()->has('parent') ? request()->get('parent') : null)) }}">
<input type='hidden' name='id' id='id' value='{!! (!empty($node->id) ? $node->id : 0) !!}' />

<!-- Name Field -->
<div class="form-group col-sm-6">
    {!! Form::label('name', 'Name:') !!}
    {!! Form::text('name', null, ['id' => 'name', 'class' => 'form-control', 'onkeyup' => 'generateSlug();', 'onblur' => 'generateSlug();', 'maxlength' => 255,'maxlength' => 255]) !!}
</div>

<!-- Slug Field -->
<div class="form-group col-sm-6">
    {!! Form::label('slug', 'Slug:') !!}
    {!! Form::text('slug', null, ['id' => 'slug', 'class' => 'form-control', 'maxlength' => 255,'maxlength' => 255]) !!}
</div>
...
@push('scripts')
   <script type="text/javascript">
           ...
           function generateSlug()
           {
               ...
               prevRequest = $.ajax({
                   method: "get", url: "/admin/nodes/generate-slug", data: "name=" + encodeURIComponent($('#name').val()) + "&parent_id=" + $("#parent_id").val() + "&id=" + $("#id").val() + "&_token=" + $('meta[name=_token]').attr('content'),
                   beforeSend : function()
                   {
                       if (prevRequest != null)
                       {
                   ...
                   success: function(data)
                   {
                       if (data.slug)
                       {
                           var slug = decodeURIComponent(data.slug);
                           $("#slug").val(slug);
                       }

                       if (data.status == "error")
                       {
                           $('#slug_err').html(data.message);
                       }
                   }
               });
           }
